...
// TRURI I AI-t (I zhvendosur nga backend-i)
...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // THIRRJA E RE DIREKT TE HUGGING FACE
      const res = await fetch("https://router.huggingface.co/v1/chat/completions", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${process.env.REACT_APP_HUGGINGFACE_API_KEY}` // Përdor çelësin nga .env
        },
        body: JSON.stringify({ 
      ...
      try {
        ...
        // PROVA 2: Nëse AI bën gabim sintakse, i gjejmë të dhënat manualisht!
      ...
      setAiData({ reply: "Ndodhi një gabim me serverin e AI-t. Të lutem provo përsëri." });
    } finally {
      setLoading(false);
    }
  };
